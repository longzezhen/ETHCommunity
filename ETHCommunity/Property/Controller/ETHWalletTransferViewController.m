//
//  ETHWalletTransferViewController.m
//  ETHCommunity
//
...
@interface ETHWalletTransferViewController ()<WKNavigationDelegate>
@property (nonatomic,strong) BaseWKWebView * webView;
@end

@implementation ETHWalletTransferViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    for (BaseViewController * vc in mutableArray) {
        if ([vc isKindOfClass:[ETHScanViewController class]]) {
...
//    NSString * urlStr = [NSString stringWithFormat:@"%@?&token=%@&language=%@",HTML_ETITWallet,TOKEN,langStr];
    NSString * urlStr = [NSString stringWithFormat:@"%@?&token=%@&otherAccounts=%@&language=%@",HTML_WalletTransfer,TOKEN,self.walletAccount,langStr];
